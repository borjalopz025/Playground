...
class threadTesstCase(TestCase):
    def setUp(self):
    ...
    def test_filters_thread_by_users(self):
        self.thread.users.add(self.user1, self.user2)
    ...
    def test_add_messages_to_thread(self):
        ...
        message2 = Message.objects.create(user=self.user2, content='buenas , que tal?')
    ...
    def test_add_message_from_user_not_in_thread(self):        
        ...
        message1 = Message.objects.create(user=self.user1, content='hola buenas')
        message2 = Message.objects.create(user=self.user2, content='buenas , que tal?')
        message3 = Message.objects.create(user=self.user2, content='Soy un espía')
